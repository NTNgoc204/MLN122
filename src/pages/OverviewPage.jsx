const overviewData = {
  ...
  lecturer: 'Gi\u1ea3ng vi\u00ean h\u01b0\u1edbng d\u1eabn: [T\u00ean gi\u1ea3ng vi\u00ean]',
  ...
  tools: [
    {
      name: 'React',
      subtitle: 'D\u00f9ng \u0111\u1ec3 x\u00e2y d\u1ef1ng giao di\u1ec7n web v\u00e0 chia component d\u1ec5 qu\u1ea3n l\u00fd.',
      icon: 'R',
      iconClass: 'from-sky-500 to-cyan-500',
    },
    {
      name: 'Canva',
      subtitle: 'D\u00f9ng \u0111\u1ec3 thi\u1ebft k\u1ebf slide, infographic v\u00e0 h\u00ecnh \u1ea3nh minh h\u1ecda.',
      icon: 'C',
    ...
    <section className="animate-fade-up animate-fade-up-delay-1 w-full px-1 sm:px-2">
      <header className="animate-fade-up animate-fade-up-delay-1 mb-11 text-center">
        <h1 className="mb-3 text-4xl leading-tight font-semibold sm:text-5xl lg:text-7xl">
          <span className="animate-gradient-shift bg-gradient-to-r from-slate-800 via-cyan-800 to-slate-800 bg-clip-text text-transparent">
            {overviewData.title}
          </span>
        </h1>
      ...
      <section className="mt-5">
        <article className="animate-fade-up animate-fade-up-delay-3 rounded-3xl border border-white/70 bg-white/65 p-6 shadow-[0_12px_24px_rgba(13,87,129,0.14)] backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_34px_rgba(13,87,129,0.2)] sm:p-8">
          <h2 className="mb-4 text-2xl font-semibold text-slate-800 sm:text-3xl">{'C\u00f4ng c\u1ee5'}</h2>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {overviewData.tools.map((tool) => (
              <article
                key={tool.name}
                className="group rounded-2xl border border-cyan-900/10 bg-white/90 p-5 shadow-[0_10px_20px_rgba(13,55,89,0.1)] transition-all duration-300 hover:-translate-y-1 hover:border-cyan-600/25 hover:shadow-[0_16px_28px_rgba(13,55,89,0.16)]"
              >
                <div className="mb-3 flex items-center gap-3">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${tool.iconClass} text-xl font-bold text-white shadow-md transition-transform duration-300 group-hover:scale-105`}
                    aria-hidden="true"
                  >
                    {tool.icon}
                  </div>
                  <h3 className="text-lg font-semibold text-slate-800 sm:text-xl">{tool.name}</h3>
                </div>
                <p className="text-base leading-relaxed text-slate-600 sm:text-lg">{tool.subtitle}</p>
              </article>
            ))}
          </div>
        </article>
